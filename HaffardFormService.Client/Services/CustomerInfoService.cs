...
namespace HaffardFormService.Client.Services
{
    public class CustomerInfoService : ICustomerInfoService
    {
        ...
        public CustomerInfoService(ILogger<CustomerInfoService> logger, IConfiguration config)
        {
            _logger = logger;
            _config = config;
        }
        ...
        public async Task<List<string>> GetDynamicFieldsByIndustryAsync(string industry)
        {
            try
            {
                ...
                var response = await Firefly.Get($"{baseUrl}/api/CustomerInfo/industry/{industry}");                

                _logger.LogInformation($"Successfully fetched dynamic fields records from the API. Response: {response}");
